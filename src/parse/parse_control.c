...
{
	if (strs_equal(str, "A") || strs_equal(str, "C") || strs_equal(str, "L")
		|| strs_equal(str, "sp") || strs_equal(str, "cy") || strs_equal(str,
			"pl"))
		return (1);
...
static int	arg_count_valid(char **arr)
{
	...
	if (strs_equal(arr[0], "C") || strs_equal(arr[0], "L") || strs_equal(arr[0],
			"sp") || strs_equal(arr[0], "pl"))
...
int	line_checker(char *line, void *ptr)
{
	...
	if (!split)
		return (-1);
	if (!object_valid(split[0]) || !arg_count_valid(split)
		|| !numbers_valid(split))
	{
		free_array(split);
		return (-1);
